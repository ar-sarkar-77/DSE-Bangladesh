...
foreach($values as $item){
    ...
    if(count($info)>=4){
        ...
        if($count>=1){
            $sql2 = "UPDATE dse_table SET price='$price' , change_price='$change_price' , change_percent='$change_percent' , time=NOW() WHERE name LIKE '$name'";
            $result2 = mysqli_query($con , $sql2);
            if($result2) echo 'Updated successful.';
        }else{
            $sql2 = "INSERT INTO dse_table(name , price , change_price , change_percent , status ) VALUES('$name' , '$price' , '$change_price' , '$change_percent' , '0' )";
            $result2 = mysqli_query($con , $sql2);
